...
phi_values = {
    ...
    #alpha , beta , gamma , depth = alptha * phi
    
    # f = d . b^phi . r^phi
    # d = depth combien de couche on a
    # beta b^phi largeur des couche
    # gamma r^phi resolution scale factor
    "b0": (0,   224, 0.2 ), 
    "b1": (0.5, 240, 0.2 ),    
    "b2": (1,   260, 0.3 ), 
    "b3": (2,   300, 0.3 ), 
...
class EfficientNet(nn.Module):
        ...
        def forward(self,x):
            x=self.features(x)
            x=self.pool(x)
            x=self.classfier(x.view(x.shape[0],-1))
            return x
...
num_exemples,num_classes = 4,10
x = torch.randn(num_exemples,3,res,res)
